...
var charts = [
    ...
    */
    ...
    },
    {
        ...
        available: function(song) { return true; },
        get: function(song) {
            return song.artist_familiarity;
        },
        nget: function(song) {
            return song.artist_familiarity;
        },
...

    {
        ...
        nget: function(song) {
            return song.audio_summary.key / 12;
        },
...

    {
        ...
        nget: function(song) {
            return song.audio_summary.time_signature / 8;
        },
    },
    {
        key: 'mode',
        ...
        nget: function(song) {
            return song.audio_summary.mode / 2;
        },
...
function getSongData(chartX, chartY, chartZ, chartW) {
    ...
    $.each(allSongs, function(index, song) {
        var d = {
            ...
            bullet: song.is_new ? 'square' : 'round',
...
function getKloutScores() {
    var artistIDs = getUniqueArtists();
...
function highlightSongByName(name) {
    ...
    function nameMatch(name, song) {
        if (name.length > 0) {
            if (song.song_name.toLowerCase().indexOf(name) >= 0) {
                return true;
            }
            if (song.artist_name.toLowerCase().indexOf(name) >= 0) {
                return true;
            }
        }
        return false;
...
function updateScatterChart() {
    ...
    var graph = new AmCharts.AmGraph();
    graph.valueField = "val";
    graph.balloonText = "[[title]] by [[artist]]";
    graph.xField = 'x';
    graph.yField = 'y';
    ...
    chart.addTitle("Song Plot", 20, "#333", true)
        // CURSOR
...
function initUI() {
    ...
    $("#more-like-this").click( function() {
        moreLikeThis(currentSong);
    });

    $("#extend-playlist").click( function() {
        extendPlaylist();
    });
...
function playSong(song) {
    if (hasTracks(song)) {
        ...
        currentSong = song;
        highlightSong(currentSong, true);
        chart.validateData();
...
function extendPlaylist() {
    en.apiRequest('playlist/static',
    {
        type:'catalog-radio',
        seed_catalog: currentTasteProfile,
        adventurousness: 1,
        variety: 1,
        bucket: ['audio_summary', 'song_hotttnesss', 'artist_hotttnesss', 'artist_familiarity', 'tracks', 'id:rdio-US'],
        results: 30,
    }, 

    function(data) {
        $.each(data.response.songs, function(i, song) {
            if (hasTracks(song) && !songExists(song)) {
                song.is_new = true;
                normalizeSong(song);
                allSongs.push(song);
            }
...
function songExists(song) {
    for (var i = 0; i < allSongs.length; i++) { 
        if (song.song_id === allSongs[i].song_id) {
            return true;
        }
    }
    return false;
...
function fetchSongsFromTasteProfile(id,start) {
    ...
    en.apiRequest('catalog/read', 
        ...
        function(data) {
            ...
            $.each(data.response.catalog.items, function(index, song) {
